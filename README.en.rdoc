...
object is {RangeExtd::NONE} or not is use of {RangeExtd#is_none?} (as
it always has been). Or, in most practical cases, {Range#null?} is
...
raised in the initialisation due to the way Ruby built-in Range is
implemented.
...
今までもずっとそうだったように、{RangeExtd#is_none?} です。実用的には、
...
* {RangeExtd::Nowhere::NOWHERE} がをRuby組込み {Range}内で用いると、
  +Range#minmax+ などは、その値を +nil+ とは認識しません(本来は認識するべき)。
