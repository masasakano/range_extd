...
<tt>Range#valid?</tt> and <tt>Range#empty?</tt>
...
Class <tt>RangeExtd::Infinity</tt> has basically only two constant instances.
...
1. Both <tt>begin</tt> and <tt>end</tt> elements must be Comparable to each other,
   ...
   The sole exception is <tt>RangeExtd::NONE</tt>, which is valid.
...
<tt>RangeExtd.valid?</tt> and <tt>Range#valid?</tt> 
...
Finally, <tt>Range#null?</tt> is equivalent to "either empty or not valid".
...
For more detail, see <tt>Range#==</tt> and <tt>RangeExtd#==</tt>, as
well as <tt>#eql?</tt>.
